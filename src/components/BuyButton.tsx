...
const BuyButton = ({
  ...
  const [totalPrice, setTotalPrice] = useState<number | unknown>();
  ...
  const callshopifyFunction = async () => {
    ...
    if (response.ok) {
      ...
      // @ts-ignore
      look_add_to_cart(allVariants, data);
  ...
  // publish(PUB_SUB_EVENTS.cartUpdate, { source: 'product-form', productVariantId:data});
  useEffect(() => {
    setTotalPrice(
  ...
  return totalPrice ? (
    <div className="loading-cart w-full" >
      {isLoader ? (
        <button
        className="button product-selector__submit no-js-hidden button--filled button--uppercase lark-berry-buy-button" id="loader-buy_look"
      >
